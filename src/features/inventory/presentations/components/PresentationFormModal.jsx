...
import { X, Save, Beaker, FileText, Activity } from "lucide-react";

const PresentationFormModal = ({ isOpen, onClose, initialData = null, mode = 'create', onSave, accentColor = "emerald" }) => {
  ...
  const handleSubmit = () => {
    ...
    if (!formData.nombre || !formData.nombre.trim()) {
      newErrors.nombre = "El nombre de la presentación es obligatorio.";
  ...
  const title = mode === 'create' ? 'Nueva Presentación' : (mode === 'edit' ? 'Editar Presentación' : 'Detalle de Presentación');
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div className={`${accent.header} px-5 py-3 border-b flex justify-between items-center`}>
          <h3 className="font-bold text-gray-800 text-sm flex items-center gap-2">
            <Beaker size={16} className={accent.text}/> {title}
          </h3>
        ...
        {isView ? (
          ...
          <div className="p-6 space-y-4">
            ...
            {/* Nombre */}
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Nombre de la Presentación</label>
              <div className="relative">
                <Beaker className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                <input
                  type="text"
